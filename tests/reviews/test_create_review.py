...
class ReviewCreateView(APITestCase):
    ...
    def test_review_creation_not_authenticadet(self):
        # STATUS CODE
        with self.subTest():
            response = self.client.post(
                self.BASE_URL_review, data={}, format="json")
    ...
    def test_review_creation_shared_with_token(self):
        ...
        with self.subTest():
            ...
            response = self.client.post(self.BASE_URL_review, data=review_data, format="json")
        ...
        expected_data = {
            ...
            "reservation": {
                ...
                "created_at": review_created.reservation.created_at.strftime('%Y-%m-%dT%H:%M:%S.%fZ'),
                "updated_at": review_created.reservation.updated_at.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
            }
        }
    ...
    def test_without_not_text(self):
    ...
    def test_without_not_stars(self):
        ...
        expected_message = {
            "stars": ["A valid integer is required."]
        }
        resulted_message = response.json()
